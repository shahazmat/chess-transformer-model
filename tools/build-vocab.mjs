...
// Appended last, so it stays a pure append that disturbs no existing id.
...
const tokens = [...cores, ...MODIFIER_TOKENS, ...NERF_TOKENS, ...STRUCTURAL_TOKENS];
...
// + ${STRUCTURAL_TOKENS.length} structural tokens (<bos>, <eos>, ${ELO_TOKENS.length} <elo-NNN> buckets, <elo-any>), in that order.
...
// were appended (vocab v2) without disturbing any pre-existing id.
...
console.log(`${tokens.length} tokens = ${cores.length} cores + ${MODIFIER_TOKENS.length} modifiers + ${NERF_TOKENS.length} nerf + ${STRUCTURAL_TOKENS.length} structural`);
